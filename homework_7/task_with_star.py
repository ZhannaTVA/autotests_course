...
class RomanNums:
    ...
    ROMAN_SYM = {"I": 1, "IV": 4, "V": 5, "IX": 9, "X": 10, "XL": 40, "L": 50, "XC": 90, "C": 100, "CD": 400,
                 "D": 500, "CM": 900, "M": 1000}
    ...
    def from_roman(self):
        ...
        while i < len(self.roman_str):
            if self.roman_str[i:i+2] in self.ROMAN_SYM.keys():
                arabic_number += self.ROMAN_SYM[self.roman_str[i:i+2]]
                i += 2
            else:
                arabic_number += self.ROMAN_SYM[self.roman_str[i]]
                i += 1
        return arabic_number

    def is_palindrome(self):
        arabic_number = self.from_roman()
        revers_number = list(str(arabic_number))
        revers_number.reverse()
        if arabic_number == int(''.join(revers_number)):
            return True
        return False
